...
public class BuslineActivity extends Activity implements OnMarkerClickListener,
	...
	private EditText selectCity;// 选择城市下拉列表
	...
	private void init() {
		...
		selectCity = (EditText) findViewById(R.id.cityName);
...
//				_b.putString("", value);
				Intent _i = new Intent(BuslineActivity.this, AtoZActivity.class);
				startActivityForResult(_i, 0x1);
			}
		});
		selectCity.setOnFocusChangeListener(new View.OnFocusChangeListener() {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(data==null)
	        return ;
	    String _c=data.getExtras().getString("city");
        //更新编辑框内容为所选的号码
        selectCity.setText(_c);
   }
	...
	public void searchLine() {
		...
		busLineQuery = new BusLineQuery(search, SearchType.BY_LINE_NAME,
				selectCity.getText().toString());// 第一个参数表示公交线路名，第二个参数表示公交线路查询，第三个参数表示所在城市名或者城市区号
		...
		busLineDialog.onListItemClicklistener(new OnListItemlistener() {
			...
			public void onListItemClick(BusLineDialog dialog,
				...
				busLineQuery = new BusLineQuery(lineId, SearchType.BY_LINE_ID,
						selectCity.getText().toString());// 第一个参数表示公交线路id，第二个参数表示公交线路id查询，第三个参数表示所在城市名或者城市区号
